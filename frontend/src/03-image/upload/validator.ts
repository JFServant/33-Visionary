...
const isImageType = (file: File): boolean => ['image/png', 'image/jpeg'].includes(file.type)
...
export const validator = (files: FileList | null): Success<File> | Failure => {
  ...
  if (!isImageType(file)) return { error: { message: 'Only PNG and JPEG images are accepted.' } }
